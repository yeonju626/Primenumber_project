...
def primes(n):
    ...
    for k in range(2, floor(sqrt(n))+1):#for every integer in the range of 2 and square root of n
        if k in primes:
            primes.difference_update( range(k**2, n+1, k) )#compares the set primes and the set of the integers whose square root is k and can be devided by k (bigger than k)                                   #and take out the repeated integer from primes
    return primes
